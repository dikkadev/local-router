...
async function loadRoutes(){
  ...
  try{
    ...
    body.replaceChildren(...routes.map(route=>{
      ...
      if(route.pinned) tr.className='pinned';
      const target=route.targetHost+':'+route.port;
      const registered=formatRegistered(route.createdAt);
      const nameCell='<span class="route-name">'+esc(route.name)+'</span>'+(route.pinned?'<span class="pin-mark" title="Pinned">pinned</span>':'');
      const statusClass=route.pinned?' pinned':'';
      tr.innerHTML='<td>'+nameCell+'</td><td><a href="'+route.url+'" target="_blank" rel="noopener noreferrer">'+route.url+'</a></td><td>'+esc(route.title||'')+'</td><td><span class="status-badge'+statusClass+'">'+esc(route.status)+'</span></td><td><span class="age">'+esc(registered.age)+'</span></td><td><span class="datetime">'+esc(registered.date)+'<br>'+esc(registered.time)+'</span></td><td>'+esc(target)+'</td><td>'+esc(route.heartbeatPath)+'</td><td>'+route.misses+'</td><td>'+esc(route.exec||'')+'</td><td class="route-actions"><button data-open="'+route.url+'">Open</button> <button data-copy="'+route.url+'">Copy</button> <button data-delete="'+route.name+'">Unregister</button> <button data-pin="'+route.name+'" data-pinned="'+route.pinned+'">'+(route.pinned?'Unpin':'Pin')+'</button> <button data-kill="'+route.name+'">Kill</button></td>';
      return tr;
    }));
